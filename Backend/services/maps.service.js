...
module.exports.getSuggestion=async(address)=>{
  ...
  const apiKey = process.env.GOOGLE_API_KEY;
  const url = `https://maps.gomaps.pro/maps/api/place/autocomplete/json?input=${encodeURIComponent(address)}&locationbias=circle:5000@37.7749,-122.4194&key=${apiKey}`;
  try {
    const response = await axios.get(url);
    if (response.data.status === 'OK') {
